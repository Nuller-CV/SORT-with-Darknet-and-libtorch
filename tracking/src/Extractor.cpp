...
namespace {
            ...
            conv = register_module(
                    ...
                    nn::Sequential(
                            nn::Conv2d(nn::Conv2dOptions(c_in, c_out, 3)
                                               ...
                                               /* .padding(1).with_bias(false)), old version */
                            nn::BatchNorm2d(c_out),
                            nn::Functional(torch::relu),
                            nn::Conv2d(nn::Conv2dOptions(c_out, c_out, 3)
                                               .stride(1).padding(1).bias(false)),
                                               /* .stride(1).padding(1).with_bias(false)), old version */
                            nn::BatchNorm2d(c_out)));
    ...
    void load_BatchNorm2d(nn::BatchNorm2d m, ifstream &fs) {
        load_tensor(m->weight, fs);
        load_tensor(m->bias, fs);
